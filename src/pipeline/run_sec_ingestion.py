"""SEC Filings Ingestion Orchestrator for Top 10 EdTech Companies.

This script orchestrates the batch ingestion of SEC filings using the existing
SEC pipeline. It fetches 10-K and 10-Q filings from the past 2 years for the
...
async def run_sec_ingestion_with_progress(tickers: List[str]) -> Dict[str, Any]:
    ...
    logger.info(f"Target filings: 10-K, 10-Q from past 2 years")
    logger.info("-" * 80)

    # Create filing requests for past 2 years
    requests = [
        FilingRequest(
